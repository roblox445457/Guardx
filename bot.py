...
class RateLimiter:
    ...
    async def is_rate_limited(self, key):
        ...
        if len(self.calls[key]) >= self.max_calls:
            ...
            if wait_time > 5:  # If wait time is too long, skip the operation
                return False
...
class DiscordBot(commands.Bot):
    def __init__(self):
        ...
        super().__init__(
            command_prefix='g!',  # Changed from '!' to 'g!'
            intents=intents,
            help_command=commands.DefaultHelpCommand()
        )

        # Rate limiter for audit log checks (max 2 per guild every 10 seconds)
        ...
        self.initial_extensions = [
            ...
            'cogs.security',
            'cogs.admin',
            'cogs.antinuke',    # New anti-nuke system
            'cogs.logging',     # Enhanced logging system
            'cogs.wordfilter',  # Word filter / curse word blocking
            'cogs.help'         # Custom help command
        ]
